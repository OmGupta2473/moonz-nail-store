        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/50">
              {['ID', 'Customer', 'Total', 'Date', 'Status', 'Actions'].map(h => (
                <th key={h} className="px-6 py-5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-gray-50">
            {orders.length === 0 ? (
              ...
              orders.map((order) => {
                const total = order.total ?? order.subtotal ?? order.totalPrice ?? 0;
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50/80 transition-colors group">
                    ...
                    <td className="px-6 py-5 text-sm text-gray-600 font-medium truncate max-w-[150px]">{customerName}</td>
                    <td className="px-6 py-5 text-sm text-gray-900 font-bold">₹{(total || 0).toFixed(2)}</td>
                    <td className="px-6 py-5 text-sm text-gray-500">{new Date(createdAt).toLocaleDateString()}</td>
                    <td className="px-6 py-5"><StatusPill status={order.status || 'Pending'} /></td>
                    <td className="px-6 py-5 flex gap-3 items-center">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => onUpdateStatus(order.id, order.userId || null, e.target.value)}
                        >
